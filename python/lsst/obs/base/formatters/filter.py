...
class FilterTranslator(StorageClassDelegate):
    ...
    def getComponent(self, composite, componentName):
        ...
        Parameters
        ----------
        composite : `~lsst.afw.image.FilterLabel`
            `Exposure` to access component.
        componentName : `str`
            Name of component to retrieve. Only "filter" is supported.
        ...
        if componentName == "filter":
            ...
            if composite == FilterLabel(band="r", physical="HSC-R2"):
                return Filter("r2", force=True)
            elif composite == FilterLabel(band="i", physical="HSC-I2"):
                return Filter("i2", force=True)
            elif composite == FilterLabel(physical="solid plate 0.0 0.0"):
                return Filter("SOLID", force=True)
            elif composite.hasBandLabel():
                return Filter(composite.bandLabel, force=True)
            else:
                # FilterLabel guarantees at least one of band or physical
                # is defined.
                return Filter(composite.physicalLabel, force=True)
        else:
